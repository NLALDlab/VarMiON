...
# Copyright (c) 2026 Laura Rinaldi
...
class pde_heat_eq__robin(PDE):
    def __init__(self, mesh, V,Q, times_train):
        ...
        self.dt = times_train[1] - times_train[0]  #self.T/self.num_steps
        ...
        #self.solution_pressure.name = 'solution_pressure'
    ...
    def solve(self):
       ...
       #boundary conditions
        ...
        def void(x):
            tol = 1.e-6
            return np.logical_and( (np.abs(x[0]- 0.2)<0.05+tol) , (np.abs(x[1]- 0.2)<0.05+tol))
        
        void_dofs = locate_dofs_geometrical(self.V, void)
        u_void = np.array((0,) * mesh.geometry.dim, dtype=PETSc.ScalarType)
        bc_void = dirichletbc(u_void, void_dofs, self.V)
...
if __name__ == '__main__':
    ...
    R = 0.4 #0.5    # half of distance between plates
    L = 2.2 #2.0    # length
    radius = 0.05
    ...
    gmsh.model.mesh.field.setNumber(thresh, "SizeMax", 0.05)   # fuori
